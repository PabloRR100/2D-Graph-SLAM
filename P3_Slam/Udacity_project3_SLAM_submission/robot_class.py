...
class robot:
    ...
    def sense(self):
        ...
        for i, l in enumerate(self.landmarks):
            ...
            lx, ly = l[0], l[1]                          # Landmark postion
            lx += self.rand() * self.measurement_noise   # Add noise so the measurement is not perfect
            ly += self.rand() * self.measurement_noise   # Add noise so the measurement is not perfect
            dx, dy = lx - self.x, ly - self.y            # Real position of the landmarks
            
            # Incorporate credibulity -- Ensure the measure is within the measurable range
            if abs(dx) < self.measurement_range and abs(dy) < self.measurement_range: 
                measurements.append([i, dx, dy])  # We append the distance beetween the landmark location and the robot location 
        return measurements
